...
Modified:
- 04/09/2017, Morten Jensen: Regularly scheduled (as opposed to SNS) etc.
'''
...
def lambda_handler(event, context):
    ...
    # Get CloudFormation stack name and add as filter
    stack_name = context.invoked_function_arn.split(':')[6].rsplit('-', 2)[0]
    SECURITY_GROUP_TAG_FOR_HTTP.update({ 'aws:cloudformation:stack-name' : stack_name })
    SECURITY_GROUP_TAG_FOR_HTTPS.update({ 'aws:cloudformation:stack-name' : stack_name })
    
    # Load the ip ranges from the url
    ip_ranges = json.loads(get_ip_groups_json(AWS_IP_RANGES_URL))

    # Simply sanity check to ensure that prefixes are there
    if len(ip_ranges['prefixes']) < 1:
        raise Exception('Suspect ip_ranges length')
